...
class BERTopic:
    ...
    def _extract_topics(self, documents: pd.DataFrame):
        ...
        """
        ## -------------------------------------------------------------------------------------------------
        # print('_extract_topics 실행중 ')
        documents_per_topic = documents.groupby(['Topic'], as_index=False).agg({'Document': ' '.join})
        # print('------------------------'*50)
        # print(type(documents_per_topic))
        # print(documents_per_topic)
        # print(documents_per_topic.info())
        # print(documents_per_topic['Document'])

         #3 제품별 키워드 불용어 설정 
        if self.product_name is None:
            pass
        elif self.product_name == '신라면':
            stopwords = ['신라면', '라면', '농심']
        elif self.product_name == '진라면':
            stopwords = ['진라면', '라면', '오뚜기']
        elif self.product_name =='푸르밀 검은콩우유':
            stopwords = ['푸르밀 검은콩우유', '검은콩우유', '검은콩', '우유', '푸르밀'] 
        else :
            print('--- Warning : New Product Detected. Need to Set Stopwords. ---')

        try:
            stopwords = '\w*\s*|\s*\w*'.join(stopwords)
            stopwords = '\s*\w*' + stopwords + '\w*\s*'

            # print('------------------------------- self.product_name : ', self.product_name)
            # print('------------------------------- stopwords : ', stopwords)

            for i in range(len(documents_per_topic)):
                doc = documents_per_topic['Document'][i]
                documents_per_topic['Document'][i] = re.sub(stopwords, ' ', doc) #4 불용어 포함한 단어 제거
        except: pass
    ...
    def _top_n_idx_sparse(matrix: csr_matrix, n: int) -> np.ndarray:
        ...
        for le, ri in zip(matrix.indptr[:-1], matrix.indptr[1:]):
            n_row_pick = min(n, ri - le)    # doc 길이와 n 중 작은 수 선택
